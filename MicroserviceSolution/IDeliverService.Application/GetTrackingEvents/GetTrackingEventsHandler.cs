...
namespace IDeliverService.Application.GetTrackingEvents
{
    ...
    public class GetTrackingEventsHandler : IRequestHandler<GetTrackingEvents, TrackingEventsResponseModel>
    {
        private readonly IIDeliverApi _iDeliverApi;

        [IntentManaged(Mode.Ignore)]
        public GetTrackingEventsHandler(IIDeliverApi iDeliverApi)
        {
            _iDeliverApi = iDeliverApi;
        }

        [IntentManaged(Mode.Fully, Body = Mode.Ignore)]
        public async Task<TrackingEventsResponseModel> Handle(GetTrackingEvents request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var response = await _iDeliverApi.GetTrackingEvents(request.Token, request.Id);
                var result = JsonConvert.DeserializeObject<TrackingEventsResponseModel>(response);
                if (result == null)
                {
